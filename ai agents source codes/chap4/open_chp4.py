...
def ollama_prompt(prompt: str, model: str = "gemma3:1b") -> str:
    try:
...
def extract_core_website_content(html: str) -> str:
    prompt = f"""
        ...
        Extract only the meaningful article or content body from the 
 	  following HTML.
        Do not include headers, footers, ads, navigation links, or 
  scripts.
        <html>
        {html}
        </html>
